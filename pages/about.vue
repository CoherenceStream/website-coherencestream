<template>
  <div class="py-16">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <h1 class="text-4xl font-bold text-gray-900 mb-8 text-center">About CoherenceStream</h1>
      
      <div class="prose prose-lg mx-auto">
        <h2 class="text-2xl font-bold text-blue-600 mb-4">Our Mission</h2>
        <p class="text-gray-700 mb-6">
          CoherenceStream is pioneering a new approach to health and data sovereignty through 
          besearch methodology and peer-to-peer networks. We believe in empowering individuals 
          with their own data while contributing to collective intelligence.
        </p>
        
        <h2 class="text-2xl font-bold text-green-600 mb-4">What is Besearch?</h2>
        <p class="text-gray-700 mb-6">
          Besearch is our revolutionary research methodology that combines:
        </p>
        <ul class="list-disc pl-6 text-gray-700 mb-6">
          <li>Individual optimization (n=1 studies)</li>
          <li>Network pattern recognition</li>
          <li>Personalized ML models</li>
          <li>Sovereign data ownership</li>
        </ul>
        
        <h2 class="text-2xl font-bold text-pink-600 mb-4">Our Technology</h2>
        <div class="grid md:grid-cols-2 gap-6 mb-8">
          <div class="card">
            <h3 class="text-xl font-semibold mb-3">HOP Protocol</h3>
            <p class="text-gray-600">
              Health Oracle Protocol enables peer-to-peer health data networks 
              without centralized platforms.
            </p>
          </div>
          <div class="card">
            <h3 class="text-xl font-semibold mb-3">BentoBoxDS</h3>
            <p class="text-gray-600">
              Open source application for health data visualization and analysis, 
              the foundation for our commercial products.
            </p>
          </div>
        </div>
        
        <h2 class="text-2xl font-bold text-blue-600 mb-4">Our Vision</h2>
        <p class="text-gray-700">
          We envision a future where individuals control their health data, contribute to 
          collective intelligence, and benefit from personalized insights - all while 
          maintaining sovereignty and privacy in a peer-to-peer network.
        </p>
      </div>
    </div>
  </div>
</template>

<script setup>
useHead({
  title: 'About - CoherenceStream',
  meta: [
    { name: 'description', content: 'Learn about CoherenceStream\'s mission to build peer-to-peer health networks using besearch methodology.' }
  ]
})
</script>